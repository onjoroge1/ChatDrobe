...
const probe=`
 ...
 if(kind==='billing'){
  ...
  for(const action of ['me','admin-setup']){const denied=await fetcher(base+'/api/account?action='+action);assert.equal(denied.status,401);assert.equal((await denied.json()).error.code,'SIGN_IN_REQUIRED');}
  const denied=await fetcher(base+'/api/account?action=admin-users',{method:'POST',headers:{origin:'https://www.chatdrobe.com','x-chatdrobe-request':'account-v1','content-type':'application/json'},body:'{}'});assert.equal(denied.status,401);assert.match(denied.headers.get('cache-control'),/no-store/);
 }
 assert.equal(outward,0);console.log('Isolated deployed handler passed: '+kind);
 }finally{await new Promise(r=>server.close(r));}
`;
for(const nested of [false,true])test('deployable billing and account bundles work from '+(nested?'nested tests':'repository')+' root',t=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'chatdrobe-deploy-'));t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));for(const name of ['scripts','src','api','server','package.json','vercel.json','site.config.json'])fs.cpSync(path.join(root,name),path.join(dir,name),{recursive:true});fs.mkdirSync(path.join(dir,'tests'),{recursive:true});fs.writeFileSync(path.join(dir,'server','.env'),'PRIVATE_CANARY=not-for-artifacts');fs.writeFileSync(path.join(dir,'server','signing-private.pem'),'PRIVATE_CANARY');
 ...
 const built=spawnSync(process.execPath,['scripts/build.mjs'],{cwd:dir,env,encoding:'utf8',timeout:15000});assert.equal(built.status,0,built.stderr);const result=buildVercelOutput(dir,{cwd:dir,env});assert.equal(result.output,path.join(invocation,'.vercel/output'));assert.deepEqual(result.functions,['/api/billing','/api/account']);const config=JSON.parse(fs.readFileSync(path.join(result.output,'config.json')));assert.equal(config.version,3);assert.equal(config.routes[0].dest,'/api/billing');assert.equal(config.routes[1].dest,'/api/account');
 for(const name of ['api','server','node_modules','contracts'])assert.equal(fs.existsSync(path.join(result.output,'static',name)),false,name);
