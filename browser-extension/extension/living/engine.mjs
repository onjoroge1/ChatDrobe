import {settings,worldById,focus,lighting,sceneLighting,stageFor,nextBoundary,state,react,placement} from './model.mjs';
...
export function createEnvironment(doc,win){
 ...
 let lastInput=Date.now(),nativeStreaming=false,bindings=false,status='Off',lastConfig='',paintKey='';
 ...
 function mount(){
  ...
  if(typeof win.ResizeObserver==='function'){resize=new win.ResizeObserver(scheduleGeometry);const main=doc.querySelector('main');if(main)resize.observe(main);const form=doc.querySelector('form:has(#prompt-textarea)');if(form)resize.observe(form);}
 ...
 function layout(){
  raf=null;if(!host)return;metrics.geometryChecks++;
  if(opts.reactions||opts.motion){const c=doc.querySelector('#thread-bottom-container')||doc.querySelector('form:has(#prompt-textarea)');if(c!==observedComposer)bindReactions();}
  const main=doc.querySelector('main');const form=doc.querySelector('form:has(#prompt-textarea)');
  const m=main?.getBoundingClientRect(),f=form?.getBoundingClientRect();
  const reading=doc.querySelector('main .markdown,main [data-message-author-role=assistant]')?.getBoundingClientRect();
  const p=placement(m,f,prefs.width,opts.view,{width:win.innerWidth,height:win.innerHeight},reading);
  const blocked=modalOpen();host.hidden=!p.visible||blocked;status=blocked?'Paused for a dialog':p.visible?opts.view==='full'?'Full World':'Portal':p.reason;
  if(!p.visible||blocked){syncQuiet();paint();return;}
  Object.assign(host.style,{left:p.left+'px',top:p.top+'px',width:p.width+'px',height:p.height+'px',borderRadius:opts.view==='portal'?'18px':'0',boxShadow:opts.view==='portal'&&!prefs?.livingCompanionOnly?'0 10px 35px #16273822':'none'});
  ...
  host.style.maskImage=opts.view==='full'?`linear-gradient(to right,#000 0px,#000 ${p.cutLeft}px,transparent ${p.cutLeft}px,transparent ${p.cutRight}px,#000 ${p.cutRight}px,#000 100%)`:'none';
  syncQuiet();paint();
 }
 ...
 function bindReactions(){
  ...
  if(!opts.reactions&&!opts.motion)return;
  for(const name of ['pointermove','keydown','pointerdown','scroll'])win.addEventListener(name,activity,{capture:true,passive:true});
 ...
 function clear(){
  ...
  quiet.configure({enabled:false});composing=false;selectionActive=false;mediaBusy=false;nativeStreaming=false;unmount();status='Off';lastConfig='';
 }
 return {configure(p,snapshot=null){
  ...
  const changed=JSON.stringify({...next,behavior:p.livingBehavior})!==lastConfig;const worldChanged=opts.world!==next.world;opts=next;world=worldById(opts.world);lastConfig=JSON.stringify({...opts,behavior:p.livingBehavior});
 ...
 },diagnostics(){return {...metrics,loaded:true,active:!!host&&!host.hidden,reason:status,world:world.id,view:opts.view,sceneNodes:scene?.nodeCount()||0,stage:stageFor(world,session),boundaryTimer:!!boundary,idleTimer:!!idleTimer,controlObserver:!!observer,reducedMotion:reduced.matches,contentReads:0,quiet:quiet.diagnostics(),companion:scene?.motionDiagnostics?.()||null};},dispose(){clear();quiet.dispose();disposed=true;}};
}
