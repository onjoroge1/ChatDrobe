...
export function createScene(doc,definition){
 ...
 let cat=null,habitat=null,pilot=null,activeAnimations=[],lastSignature='',held=null,disposed=false;
 ...
 return {...scene,
  setCompanionOnly(){pilot?.destroy();pilot=null;const backdrop=scene.element.querySelector(':scope > svg');backdrop?.remove();scene.element.style.background='transparent';scene.element.dataset.companionOnly='true';delete scene.element.dataset.artwork;},
  ...
  motionDiagnostics(){return {...(cat?.diagnostics()||{animations:activeAnimations.length}),pilot:pilot?.diagnostics()||null};},
  dispose(){disposed=true;cancelDrone();cat?.destroy();pilot?.destroy();pilot=null;},
