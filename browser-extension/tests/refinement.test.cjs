...
test('eighteen kilobyte eager-JS budget excluding opt-in idle code',()=>{const m=JSON.parse(fs.readFileSync(path.join(base,'manifest.json')));const bytes=m.content_scripts[0].js.reduce((n,f)=>n+fs.statSync(path.join(base,f)).size,0);assert.ok(bytes<18000,`${bytes} bytes`);});
