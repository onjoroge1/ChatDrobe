/* Build the runtime from versioned local palettes and original SVG artwork. */
...
if(JSON.parse(fs.readFileSync(path.join(base,'package.json'),'utf8')).version!==version)throw new Error('Extension package version differs from release.json.');
...
if(manifest.version!==version||JSON.stringify(manifest.optional_host_permissions)!==JSON.stringify([origin+'/*']))throw new Error('Unexpected release version or account-integration manifest contract.');
...
console.log(`Built ChatDrobe ${version}: ${themes.length*2} palette variants and original artwork.`);
