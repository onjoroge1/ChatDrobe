...
The versioned Manifest V3 runtime is in `extension/`. This source was imported from the audited 0.7.0 source archive; 0.8.0 repairs selection, motion, visibility, saved appearance, account handoff, and access refresh. `../release.json` is the release version authority. The package and Chrome manifest must match it or the build fails.
