...
const output=path.join(root,'dist');fs.rmSync(output,{recursive:true,force:true});fs.mkdirSync(path.join(output,'assets'),{recursive:true});
...
for(const file of ['styles.css','client.js'])fs.copyFileSync(path.join(root,'src',file),path.join(output,'assets',file));
...
const routes=[];
function page(route,title,body,summary){routes.push(route);write(route==='/'?'index.html':route.slice(1)+'index.html',layout({title,summary,path:route,body,config}));}
