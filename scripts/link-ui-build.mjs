...
export function installLinkUi(root,output){
 ...
 for(const route of ['signup','signin','account','admin']){const file=path.join(output,route,'index.html');let html=fs.readFileSync(file,'utf8');html=html.replace('</head>','<script type="module" src="/assets/account-link-ui.js"></script></head>');fs.writeFileSync(file,html);}
