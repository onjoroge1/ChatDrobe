...
test('every generated beta banner distinguishes Free from verified Premium account access',()=>{
 for(const route of [...manifest.routes,'/404/']){
  ...
  assert.ok(banner,`Missing beta banner: ${route}`);assert.ok(banner.includes(`${freeCount} Free worlds`),route);assert.match(banner,/Premium account access/,route);assert.doesNotMatch(banner,/All \d+ worlds unlocked/i,route);
 }
});
