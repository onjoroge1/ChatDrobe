...
const freeCount = themes.filter(theme => theme.plan === 'free').length;
...
test('every generated beta banner distinguishes Free from explicit Premium tester access', () => {
  for (const route of [...manifest.routes, '/404/']) {
    ...
    const banner = html.match(/<div class="beta-bar">([\s\S]*?)<\/div>/)?.[1];
    assert.ok(banner, `Missing beta banner: ${route}`);
    assert.ok(banner.includes(`${freeCount} Free worlds`), route);
    assert.match(banner, /Premium tester preview/, route);
    assert.doesNotMatch(banner, /All \d+ worlds unlocked/i, route);
  }
});
