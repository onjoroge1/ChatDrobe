...
These must be real, configured server values, not values pasted into GitHub or the extension. Connecting a ChatGPT plugin does not automatically set the deployed application's environment. /api/account?action=status reports whether email settings are configured; it does not claim an email was delivered. Without configuration the form stays unavailable rather than accepting unverifiable admin access. No real email is sent by tests.
...
Primary references: https://resend.com/docs/api-reference/emails/send-email ; https://docs.stripe.com/webhooks ; https://docs.stripe.com/api/webhook_endpoints ; https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html
