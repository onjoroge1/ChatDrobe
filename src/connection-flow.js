...
export function connectionState(profile,linked,signing,returned=false,verificationFailed=false){
...
if(typeof document!=='undefined'){
 ...
 if(root){
  ...
  const content=root.querySelector('[data-account-content]');let serial=0,busy=false,timer=null,attempts=0,stopped=false,queued=false;
  ...
  async function update(reconcile=false){
   ...
   try{
    ...
    const linked=devices.devices?.length>0&&!new URLSearchParams(location.hash.slice(1)).get('link'),s=connectionState(profile,linked,diagnostic.signing,returned,!!verificationError);
