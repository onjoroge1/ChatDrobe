...
export function explorerContent(html,route,themes){
 ...
 html=html.replaceAll('All 12 worlds unlocked',`${free} Free worlds · Premium tester preview`)
  .replaceAll('All 12 themes are unlocked in this beta.',`${free} Free worlds are available without a subscription. Premium can be enabled explicitly for private testing.`)
  .replaceAll('All 12 original worlds are unlocked in the private beta.',`${free} original worlds are Free. Four Premium worlds require the explicit private tester preview; no paid checkout is active.`)
  .replaceAll('All 12 worlds are unlocked.',`${free} worlds are Free; Premium requires the explicit tester preview.`)
  ...
  .replaceAll('All unlocked in the private beta.','Free starter worlds plus a Premium tester collection.')
  .replaceAll('Unlocked now','Free world / Premium tester preview')
  ...
  .replaceAll('>Unlocked</td>',`>${free} Free + Premium tester preview</td>`)
  .replaceAll('Try the unlocked beta','Try the private beta')
  .replaceAll('Extension 0.2.0 · Private beta','Extension 0.4.0 · Private beta')
  .replaceAll('extension beta 0.2.0','extension beta 0.4.0');
 if(route==='/'){
  const collection=`<section class="section wrap explorer-collection"><div class="section-heading"><div><p class="eyebrow">NEW · THE EXPLORER COLLECTION</p><h2>A garage. A bridge.<br>A whole solar system.</h2></div><p>Three new Free worlds with bright default palettes. Original miniatures. Optional Premium idle routines. Your conversations stay center stage.</p></div><div class="world-grid">${ids.map(id=>card(themes.find(t=>t.id===id))).join('')}</div><p class="micro">Requires extension 0.4.0. Motion is off by default. Premium testing is opt-in; checkout is not connected.</p></section>`;
  html=html.replace('<section class="proof-strip',collection+'<section class="proof-strip');
 }
 const id=ids.find(id=>route===`/themes/${id}/`);
 if(id){
  const [name,description]=routines[id];
  const details=`<section class="world-behavior notice-box"><h2>${name}</h2><p>${description}</p><p>The light/dark world is Free. Its routine is a Premium tester feature in extension <strong>0.4.0</strong>: About → Enable private tester preview, then Play → This world’s character → World routine.</p><p>Motion starts only after you enable it and become idle. These new routines use clear page margins, skip narrow layouts, stop on input, and do not inspect message text. Static appearance files never enable motion or grant Premium access.</p></section>`;
