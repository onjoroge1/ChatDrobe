...
function summaries(config){return {
 '/':'Living worlds for your ChatGPT workspace: Rainy Tokyo Loft, Starship Journey and Cozy Train Journey. Explore 15 static themes plus three Plus environments.',
 '/themes/':'Explore three Plus Living environments and 15 static ChatDrobe themes. Browse Rainy Tokyo Loft, Starship Journey and Cozy Train Journey, or save a simpler look.',
 '/install/':'Install the ChatDrobe private beta in desktop Chrome, enter a Living World, import static appearance files and troubleshoot setup.',
 '/how-it-works/':'Learn how ChatDrobe combines appearance themes and Living environments with side-panel prompts, notes, reading controls and a focus timer.',
 '/pricing/':`ChatDrobe Free includes 11 static themes. Plus is $${config.plusAnnualUsd}/year with the complete theme collection and three Living environments. Checkout is not enabled yet.`,
 '/help/':'Troubleshoot ChatDrobe setup, Living Worlds and static appearance imports. Find privacy-conscious bug-report guidance and public GitHub support.',
 ...
 '/architecture/':'Explore the website and extension architecture, CSS-first integration, local side-panel tools and performance testing boundaries.',
 ...
 '/beta-use/':'Understand the ChatDrobe private beta, local-data limitations, artwork credits, independent branding and support boundaries.',
 '/404/':'This ChatDrobe page could not be found. Return home or explore the original theme worlds.'
};}
const betaContent='<p>This is a test release, not a paid service agreement. These notes describe current product boundaries; final commercial terms require review before paid launch.</p><h2>What the beta does</h2><p>ChatDrobe changes presentation and provides local tools. It is independent of OpenAI, does not provide ChatGPT access and does not improve or guarantee model answers. Original artwork and selected reusable artwork are included.</p><h2>Keep a backup</h2><p>Unpacked extensions and their local storage can change or be removed. Do not rely on the scratchpad as your only copy of important information. The website does not back up or synchronize extension data.</p><h2>Support and known limitations</h2><p>Desktop Chrome and signed-in ChatGPT compatibility are still being tested. No uptime commitment, completion date or premium feature delivery is promised. Follow the <a href="/help/">troubleshooting guide</a> and avoid sharing private chats in public reports.</p><h2>No payment collected</h2><p>The website displays the Free and Plus plans. Checkout is not enabled yet. There is currently no purchase, subscription, renewal or cancellation process on this website. Billing will require separate disclosures and working cancellation controls before launch.</p><h2>Artwork and source</h2><p>The Tokyo reading nook includes selected CC0 Sprouts artwork from <a href="https://www.dicebear.com/styles/sprouts/">DiceBear</a>. Its optional steam animation uses the MIT-licensed <a href="https://github.com/airbnb/lottie-web">Lottie Web</a> light player. Source versions, licenses and reproduction steps are recorded in the product repository. The cat companion remains original ChatDrobe artwork.</p><p>Making source publicly visible does not grant an open-source license. No third-party franchise character license is claimed. ChatDrobe brand and domain clearance remain launch requirements.</p>';
export function finalizeSite(output,config,routes,env=process.env){
 const settings=releaseSettings(config,env),route='/beta-use/',descriptions=summaries(config);
 ...
 for(const r of [...routes,'/404/']){
  ...
  const desc=descriptions[r]?escape(descriptions[r]):html.match(/<meta name="description" content="([^"]*)"/)[1];
  ...
  // Access wording must remain truthful regardless of earlier count rewrites.
  fs.writeFileSync(file,html.replace(/All \d+ worlds unlocked/g,`${freeCount} Free worlds · Premium account access`));
 }
